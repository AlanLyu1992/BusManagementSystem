var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
    ...
    renderGroup: function (target, groupIndex, group, frozen) {
        ...
        var groupFoot = opts.groupFootField;
        var table = [];
        ...
        for (var j = 0; j < group.rows.length; j++) {
            ...
            table.push('</tr>');
            index++;
            if (j == group.rows.length - 1) {
                if (groupFoot != undefined) {
                    var footvalue;
                    var footfield;
                    var rowId = state.rowIdPrefix + '-' + (frozen ? 1 : 2) + '-' + index;
                    table.push('<tr id="_' + rowId + '" datagrid-row-index="' + index + '" >');
                    for (k = 0; k < fields.length; k++) {
                        if (groupFoot[0][0].OperationColumn == fields[k]) {
                            table.push('<td field="' + groupFoot[0][0].OperationColumn + '"  style="height:25px">');
                            table.push('<div style="text-align:' + groupFoot[0][0].Align + ';height:auto;" class="datagrid-cell datagrid-cell-c2-' + groupFoot[0][0].OperationColumn + '">' + groupFoot[0][0].ShowContent + '</div>');
                            table.push('</td>');
                        }
                        else {
                            footfield = fields[k];
                            for (x = 0; x < groupFoot[1].length; x++) {
                                if (footfield == groupFoot[1][x]) {
                                    footvalue = 0.00;
                                    for (t = 0; t < group.rows.length; t++) {
                                        footvalue = parseFloat(footvalue) + parseFloat(group.rows[t][footfield]);
                                    }
                                    footvalue = parseFloat(footvalue).toFixed(2);
                                    break;
                                }
                                else {
                                    footvalue = '';
                            ...
                            table.push('<td field="' + footfield + '"  style="height:25px">');
                            table.push('<div style="text-align:right;height:auto;" class="datagrid-cell datagrid-cell-c2-' + footfield + '">' + footvalue + '</div>');
                            table.push('</td>');
                        }
                    }
                    table.push('</tr>');
        ...
        function initCss() {
            if (!$('#datagrid-group-style').length) {
                $('head').append(
					...
					'.datagrid-group{height:25px;overflow:auto;font-weight:bold;border-bottom:1px dotted #ccc;width:100%;}' +
					'</style>'
				);
            }
        }
