...
function showMenu(ctrlid, align, click, offset, duration, timeout, layer, showid, maxh, drag) {
    ...
    var showobj = $Y(showid);
    var menuobj = $Y(showid + '_menu');
    ...
    if (is_ie && is_ie < 7) {
        if (!jsmenu['iframe'][layer]) {
            ...
            $Y('append_parent') ? $Y('append_parent').appendChild(iframe) : menuobj.parentNode.appendChild(iframe);
...
function hideMenu(layer) {
    ...
    if (jsmenu['active'][layer]) {
        try {
            $Y(jsmenu['active'][layer].ctrlkey).className = ctrlobjclassName;
        } catch (e) { }
...
function SaveFile(ATTACHMENT_ID, ATTACHMENT_NAME) {
    URL = "/Attachment/RotateStore/Step1.aspx?attachid=" + ATTACHMENT_ID + "&attachname=" + ATTACHMENT_NAME + "&A=1";
...
function sel_attach(div_id, dir_field, name_field, disk_id, filter) {
    var hidpostfix = document.getElementById("hidpostfix");
    if (hidpostfix)
        hidpostfix.value = div_id;
    if (!filter)
        filter = "";
    var URL = "/Attachment/SelectFile/SelectFile.aspx";
...
//    if (file.Size * 1 > attachMaxLength * 1) {
//        alert("文件不能超过" + attachMaxLength + "M!");
//    }
    ...
    for (var i = 0; i < el.length; i++) {
        ...
        var elprefix = el[i].id.substring(0, file.id.lastIndexOf("_"));
        if (!elprefix)
            elprefix = "ATTACHMENT";

        if (elprefix == prefix && el[i].tagName && el[i].tagName.toLowerCase() == "input" && el[i].type.toLowerCase() == "file" && el[i].id != file.id && el[i].value == file.value) {
            alert("该文件已经添加");
            addFileLink.removeChild(file);
            var attach = CreateFileEl(file.id, file.onchange);
            addFileLink.appendChild(attach);
    ...
    attach_div.innerHTML += "<span id='" + prefix + "_span_" + id + "' title='" + file.value + "'><img src='/theme/default/images/attach.png' align='absMiddle'>" + attach_name + "<img src='/theme/default/images/remove.png' onclick='RemoveFile(this)' align='absMiddle' style='cursor:hand;'>;&nbsp;</span>";
    file.style.zIndex = "-1";
...
//postfix为前缀，一个页面有多个控件时用于区分 每个不同的控件
//show_sel_attach是否显示从文件柜和网络硬盘选择附件
function ShowAddFile(postfix, show_sel_attach) {
    if (isUndefined(postfix)) postfix = "";
    document.write('<span id="ATTACHMENT' + postfix + '_div"></span><span id="ATTACHMENT' + postfix + '_upload_div" style="display:none;"></span><div id="SelFileDiv' + postfix + '"></div><a class="addfile" href="javascript:;">' + ButtonText + '<input class="addfile" type="file" name="ATTACHMENT' + postfix + '_0" id="ATTACHMENT' + postfix + '_0" size="1" hideFocus="true" onchange="AddFile(this);" /></a>');
    if (show_sel_attach != '0')
        document.write('&nbsp;|&nbsp;<a href="#" onclick="sel_attach(\'' + postfix + '\');" class="selfile">从文件柜和网络硬盘选择附件</a><input type="hidden" value="" name="ATTACH_NAME' + postfix + '" id="ATTACH_NAME' + postfix + '"><input type="hidden" value="" name="ATTACH_DIR' + postfix + '" id="ATTACH_DIR' + postfix + '"><input type="hidden" value="" name="DISK_ID' + postfix + '" id="DISK_ID' + postfix + '">');
    if (!document.getElementById("hidpostfix"))
        document.write('<input id="hidpostfix" type="hidden" value="' + postfix + '" />');
    //用于保存从文件柜和网络硬盘选择附件选择的文件
    /*
    网络硬盘：ATTACH_DIR文件目录 ATTACH_NAME文件名 DISK_ID网络硬盘ID
    个人文件柜：ATTACH_DIR附件ID ATTACH_NAME文件名 DISK_ID 固定为personFolder
    */
}