function LoadScript(url) {
    ...
    //查找页面中的script，如果脚本路径与rul不同则添加
    for (var i = 0; i < scripts.length; i++) {
        var src = scripts[i].getAttribute('src');
        //防止<script />没有属性src的情况报错
...
LoadScript('/Script/js/jquery.min.js');
LoadScript('/Script/js/jquery-ui.custom.min.js');
LoadScript('/Script/js/jquery.cookie.js');
LoadScript('/Script/js/jquery.dynatree.min.js');
...
Tree.prototype.BuildTree = function () {
    ...
    jQuery("#" + this.id + " li").live('mouseenter', function () {
        ...
        //                jQuery.get('/general/ipanel/user/user_ip.php', { SHOW_IP: (node.data.show_ip ? '1' : ''), UID: node.data.uid }, function (data) {
...
Tree.prototype.redrawNode = function (id, action, text, new_id, new_url) {
    ...
    if (action == "add" || action == "copy" || action == "cut" || action == "share") {
        ...
        this.editNode({ key: id, title: text, new_key: new_id, url: new_url });
    }
}
