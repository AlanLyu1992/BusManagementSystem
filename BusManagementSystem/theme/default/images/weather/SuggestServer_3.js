var SuggestServer = function () {
	...
	this._position = function () {
		...
		var pos = findElementPos(__elementTarget);
		__intPositionLeft = pos[0];
		__intPositionTop = pos[1];
		...
		if (this._objectConfig["pos"])
		{
			this._elementContainer.style.top = this._objectConfig["pos"][1]+"px";
			this._elementContainer.style.left = this._objectConfig["pos"][0]+"px";
			return ;
		}
	...
	this._fill = function (defaultData) { // modify by chenliang
		var __stringValue = this._elementInput.value;
		if (("key_" + __stringValue) in this._objectData && this._objectData["key_" + __stringValue] != "" || defaultData) {// modify by chenliang
			...
			this._position();
			var __stringTable = '';
			__stringTable += '<table style="border-collapse:collapse; line-height:18px; border:1px solid #B3DBF2; background-color:#FFF; font-size:12px; text-align:center; color:#666; width:' + (this._objectConfig["width"] - 2) + 'px;"><caption style="border:1px solid #B3DBF2;border-bottom:0;background:#fff;text-align:left;">'+this._objectConfig["tip"]+'</caption>'
			if (this._objectConfig["head"] != null) {
				__stringTable += '<tr style="background-color:#6699cc;color:#fff;">';
			...
			var __arrayData ;
			if (defaultData){//modify by chenliang
				__arrayData = defaultData.replace(/&amp;/g, "&").replace(/;$/, "").split(";");
			}
			else
			{
				__arrayData = this._objectData["key_" + __stringValue].replace(/&amp;/g, "&").replace(/;$/, "").split(";");
			}
			...
			for (var i = 0; i < __intMax; i++) {
				...
				if (this._objectConfig["link"] == null || this._objectConfig["link"] == "") {
					...
					var __arrayTd = ['<td style="padding:0px;"><a href="' + __stringHref + '" hidefocus="true" onmousedown="return this.parentNode.parentNode.' + __stringSuggest + '[\'hidepause\'](this);" onclick="return this.parentNode.parentNode.' + __stringSuggest + '[\'hideresume\'](this);" style="color:#666; display:block; outline:none; padding:1px; text-decoration:none; width:100%;" target="' + this._objectConfig["target"] + '">', '</a></td>'];
				}
	...
	this._eventFocus = function () {
		...
		// modify by chenliang 0319 
		//目的:实现当输入框没有值时，显示默认的推荐城市 
		if (this._elementInput.value== "")
		{
			if (this._elementInput.getAttribute("data-default"))
			{
				var _default_data = this._elementInput.getAttribute("data-default");
				this._fill(_default_data);
			}
		}
		//end 
		this._stringLastValue = "";
