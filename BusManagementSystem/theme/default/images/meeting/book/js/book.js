...
function WeekBooker(cfg) {
  ...
  var name = $("<div class='axis-y-name'></div>").html("<span style='font-size:10pt;'>" + c.name || '' + "</span>").height(height);
  ...
  $.each(c.date, function(i, e) {
      axis.append("<div class='item " + (i % 2 ? "odd" : "even") + "' ><span style='font-size:10pt;'>" + (e.date || e) + "</span></div>");
  ...
  function add(meeting) {
    ...
    meeting.disabled || me.resizable({
      ...
      stop: function(e) {
        ...
        if (crowd()) {
            jQuery.messager.alert("提示", "会议时间有冲突", "info");
        }
      }
    }).draggable({
...
function Meeting(cfg) {
  ...
  var tr = $("<tr></tr>");
  var tdl = $("<td class='start-time' style='font-size:10pt;'></td>");
  var tdc = $("<td></td>");
  var tdr = $("<td class='end-time' style='font-size:10pt;'></td>");
  var disabled = c.disabled;
  disabled && el.addClass("disabled");
  var times = $("<p class='time-info' style='font-size:10pt;'></p>");
  var content = $("<p class='content' style='font-size:10pt;'></p>").text(c.content);
  ...
  //初始化时间显示
  function doTimes() {
      tdl.html("<p class='date' style='font-size:11pt;'>" + c.day + "</p><p class='time' style='font-size:11pt;'>" + mins2Time(c.start) + "</p>");
      tdr.html("<p class='date' style='font-size:11pt;'>" + c.day + "</p><p class='time' style='font-size:11pt;'>" + mins2Time(c.start + c.limit) + "</p>");
    if (c.status >= 4) {
